...
class PiperEngine(private val context: Context) {
    ...
    fun synthesizeTestTone(): ShortArray {
        ...
        // Keep the result as an explicit value. Kotlin cannot infer the
        // generic return type of use{} for ONNX Runtime's overloaded run(),
        // and a non-local return from that lambda is not allowed here.
